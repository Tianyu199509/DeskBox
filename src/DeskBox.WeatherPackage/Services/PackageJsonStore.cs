...
internal enum PackageJsonLoadSource
{
    Primary,
    Backup,
    DefaultMissing,
...
internal sealed record PackageJsonLoadResult<T>(
    T Value,
    PackageJsonLoadSource Source);

internal static class PackageJsonStore
{
    ...
    public static async Task<T> LoadAsync<T>(
        ...
        string logName)
    {
        PackageJsonLoadResult<T> result = await LoadWithResultAsync(
            storePath,
            deserialize,
            createDefault,
            logName);
    ...
    public static async Task<PackageJsonLoadResult<T>> LoadWithResultAsync<T>(
        string storePath,
        Func<string, T> deserialize,
        Func<T> createDefault,
        string logName)
    {
        ...
        if (File.Exists(storePath))
        {
            try
            {
                return new PackageJsonLoadResult<T>(
                    deserialize(await File.ReadAllTextAsync(storePath)),
                    PackageJsonLoadSource.Primary);
            }
            catch (Exception ex)
            {
                primaryFailed = true;
                PackageLogger.Log($"[{logName}] Primary store is invalid: {ex}");
        ...
        if (!File.Exists(backupPath))
        {
            return new PackageJsonLoadResult<T>(
                createDefault(),
                primaryFailed
                    ? PackageJsonLoadSource.DefaultAfterFailure
                    : PackageJsonLoadSource.DefaultMissing);
        }
        ...
        try
        {
            ...
            try
            {
                ...
                PackageLogger.Log($"[{logName}] Restored store from '{backupPath}'.");
            }
        ...
        {
            PackageLogger.Log($"[{logName}] Backup store is invalid: {ex}");
            return new PackageJsonLoadResult<T>(
                createDefault(),
                PackageJsonLoadSource.DefaultAfterFailure);
        }
        ...
        for (int retryIndex = 0; ; retryIndex++)
        {
            ...
            catch (IOException ex) when (IsUnableToRemoveReplacedFile(ex))
            {
                if (retryIndex < s_replaceRetryDelays.Length)
                {
                    ...
                    PackageLogger.Log(
                        $"[PackageJsonStore] Atomic replace could not remove the " +
                ...
                PackageLogger.Log(
                    $"[PackageJsonStore] Atomic replace remained unavailable; " +
    ...
    private static void QuarantineCorruptFile(string storePath, string logName)
    {
        ...
        try
        {
            ...
            PackageLogger.Log($"[{logName}] Preserved corrupt store as '{corruptPath}'.");
        }
        catch (Exception ex)
        {
            PackageLogger.Log($"[{logName}] Failed to quarantine corrupt store: {ex}");
        }
    }
