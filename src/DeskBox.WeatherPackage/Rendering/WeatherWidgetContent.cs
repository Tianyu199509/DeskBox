...
/// </summary>
public sealed partial class WeatherWidgetContent : UserControl, IDisposable
{
    ...
    public WeatherWidgetContent(WeatherWidgetViewModel viewModel, string packageRoot)
    {
        _viewModel = viewModel;
        InitializeView(packageRoot);
        DataContext = _viewModel;
        _surface.DataContext = _viewModel;
        ...
        FindRefreshIcons();
        // RadioButtons defaults to index 0 independently of DataContext. Keep
        // its native selection aligned before the first Loaded/reparent cycle.
    ...
    private void RefreshWeatherTextScaleFactor()
    {
        _viewModel.UpdateSystemTextScaleFactor(
            PackageEnvironment.ResolveSystemTextScaleFactor());
    }
    ...
    {
        if (_surface.Resources.TryGetValue(resourceKey, out object? resource) &&
    ...
    {
        if (e.PropertyName == nameof(WeatherWidgetViewModel.HourlyForecastItemsSource))
        {
            RefreshForecastItems(hourly: true);
            return;
        }
        if (e.PropertyName == nameof(WeatherWidgetViewModel.DailyForecastItemsSource))
        {
            RefreshForecastItems(hourly: false);
            return;
        }
    ...
    private void UpdateRichSkinTextTheme()
    {
        ...
        RequestedTheme = PackageEnvironment.Theme;
        RootGrid.RequestedTheme = PackageEnvironment.Theme;
    }
    ...
    {
        if (!_isSynchronizingViewSelection &&
    ...
    {
        return PackageEnvironment.ShouldAnimate;
    }
}
