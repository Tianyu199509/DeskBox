...
public sealed partial class MusicWidgetViewModel
{
    private async Task ApplyInfoAsync(MusicSessionInfo? info, int? mediaPropertiesGeneration = null)
    {
        ...
        _lastPositionSyncAt = DateTimeOffset.UtcNow;
            ...
            _coverRetryCount = 0;
            SetArtworkColor(MusicColors.DefaultAccent, hasArtworkColor: false);
            CanPlay = false;
            CanPause = false;
            CanGoPrevious = false;
            CanGoNext = false;
        ...
        if (coverChanged)
        {
            ...
            if (cover is not null)
            {
                ...
                SetArtworkColor(artworkColor ?? MusicColors.DefaultAccent, artworkColor.HasValue);
    ...
    {
        if (!Environment.UsesSystemAccentColor) return Environment.Accent;
        return UseArtworkBackdrop && _hasArtworkColor ? _artworkColor : Environment.Accent;
    }
    ...
    private async Task<(BitmapImage? image, Color? color)> LoadThumbnailAndColorAsync(MusicSessionInfo info)
    {
        ...
        {
            PackageLog.Write($"[MusicWidget] Failed to load thumbnail/color: {ex.Message}");
            return (null, null);
        }
    }
    ...
    private static async Task<Color?> ExtractDominantColorAsync(IRandomAccessStream stream)
    {
        ...
        {
            PackageLog.Write($"[MusicWidget] Failed to sample thumbnail color: {ex.Message}");
            return null;
        }
    }
}
