...
public sealed partial class MusicWidgetViewModel : ObservableObject, IDisposable
{
    ...
    private readonly MusicStrings _localizationService;
    private readonly MusicEnvironmentContext? _settingsService;
    private readonly MusicInstance _config;
    ...
    private double _textSize = MusicEnvironmentContext.DefaultTextSize;
    ...
    private string _displayMode = MusicEnvironmentContext.MusicDisplayModeAuto;
    private Color _artworkColor = MusicColors.DefaultAccent;
    private bool _hasArtworkColor;
    private MusicPlaybackMode _playbackMode = MusicPlaybackMode.Normal;

    public MusicWidgetViewModel(
        MusicInstance config,
        MusicSessionService musicSessionService,
        MusicStrings localizationService,
        MusicEnvironmentContext? settingsService = null,
        ...
        MusicVolumeService? musicVolumeService = null)
    {
        ...
        {
            _textSize = MusicEnvironmentContext.NormalizeTextSize(_settingsService.Settings.TextSize);
    ...
    internal int ActiveTimerCount { get; private set; }
    internal int CoverDecodeCount { get; private set; }
    internal int ProgressTimerIntervalMs { get; private set; }
    internal MusicEnvironmentSnapshot Environment => _settingsService?.Settings ?? new();
    ...
    {
        get => _displayMode;
        private set => SetProperty(ref _displayMode, MusicEnvironmentContext.NormalizeMusicDisplayMode(value));
    }
    ...
    public double TitleTextSize => Math.Min(MusicEnvironmentContext.MaxTextSize + 3, TextSize + 4);

    public double MinimalTitleTextSize => Math.Min(MusicEnvironmentContext.MaxTextSize + 1, TextSize + 1.5);

    public double SecondaryTextSize => Math.Max(MusicEnvironmentContext.MinTextSize, TextSize - 1);

    public double CaptionTextSize => Math.Max(MusicEnvironmentContext.MinTextSize - 1, TextSize - 2);
    ...
    private void ScheduleDebouncedMediaRefresh()
    {
        ...
        {
            MusicUiContext.TryEnqueue(_dispatcherQueue, () => _ = RunDebouncedMediaRefreshAsync(gen));
            return;
        }
    ...
    private void OnTimelineChanged(object? sender, EventArgs e)
    {
        ...
        if (_dispatcherQueue is not null && !_dispatcherQueue.HasThreadAccess)
        {
            MusicUiContext.TryEnqueue(_dispatcherQueue, () => _ = RefreshTimelineAsync());
            return;
        }
    ...
    private void OnPlaybackInfoChanged(object? sender, EventArgs e)
    {
        ...
        if (_dispatcherQueue is not null && !_dispatcherQueue.HasThreadAccess)
        {
            MusicUiContext.TryEnqueue(_dispatcherQueue, () => _ = RefreshPlaybackAsync());
            return;
        }
    ...
    private void ScheduleFullRefresh()
    {
        ...
        if (_dispatcherQueue is not null && !_dispatcherQueue.HasThreadAccess)
        {
            MusicUiContext.TryEnqueue(_dispatcherQueue, () => _ = RefreshAsync());
            return;
        }
    ...
    private async Task RefreshTimelineAsync()
    {
        ...
        {
            if (!_isDisposed)
                PackageLog.Write($"[MusicWidget] Timeline refresh failed: {ex.Message}");
        }
    }
    ...
    private async Task RefreshPlaybackAsync()
    {
        ...
        {
            if (!_isDisposed)
                PackageLog.Write($"[MusicWidget] Playback refresh failed: {ex.Message}");
        }
    }
    ...
    {
        ActiveTimerCount = _progressTimer?.IsRunning == true ? 1 : 0;
        ProgressTimerIntervalMs = _progressTimer is null
            ? 0
    ...
    private void OnSettingsChanged()
    {
        ...
        if (_dispatcherQueue is not null && !_dispatcherQueue.HasThreadAccess)
        {
            MusicUiContext.TryEnqueue(_dispatcherQueue, OnSettingsChanged);
            return;
        }
    ...
    private void ApplyMusicSettings(MusicEnvironmentSnapshot settings)
    {
        // The per-kind store is the authoritative source; the legacy
        // MusicEnvironmentSnapshot fields are a mirror that the global debounce persists.
        ...
        MusicWidgetSettings musicSettings = settings.Music;
    ...
    {
        return Environment.CornerRadius;
    }
