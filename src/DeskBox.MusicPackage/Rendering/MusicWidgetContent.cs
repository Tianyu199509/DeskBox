...
public sealed partial class MusicWidgetContent : UserControl, IDisposable
{
    ...
    private Grid? _dragProgressHost;
    ...
    public MusicWidgetContent(string packageRoot)
    {
        try { InitializeView(packageRoot); }
        catch { DetachViewEvents(); Content = null; throw; }
        ...
        WireRecordProgress(RecordProgressHost);
        WireRecordProgress(RecordHorizontalProgressHost);
    }

    public MusicWidgetContent(string packageRoot, MusicWidgetViewModel viewModel)
        : this(packageRoot)
    {
        ViewModel = viewModel;
    }
    ...
    private async void SourceButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            PackageLog.Write($"[MusicWidget] Show source picker failed: {ex}");
            CloseSourceFlyout();
        }
    }
    ...
    private async void VolumeButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            PackageLog.Write($"[MusicWidget] Show inline volume failed: {ex}");
            InlineVolumePanel.Visibility = Visibility.Collapsed;
        }
    }
    ...
    private void ProgressHost_PointerPressed(object sender, PointerRoutedEventArgs e)
    {
        ...
        _dragProgressHost = (Grid)sender;
        _dragProgressHost.CapturePointer(e.Pointer);
        ViewModel.BeginSeek();
    ...
    private async void ProgressHost_PointerReleased(object sender, PointerRoutedEventArgs e)
    {
        ...
        _dragProgressHost?.ReleasePointerCapture(e.Pointer);
        _dragProgressHost = null;
        UpdateProgressVisuals();
        e.Handled = true;
    }
    ...
    private void ApplyResponsiveLayout(double width, double height)
    {
        ...
        string normalizedMode = MusicEnvironmentContext.NormalizeMusicDisplayMode(ViewModel?.DisplayMode);
        bool isRecordVertical = normalizedMode == MusicEnvironmentContext.MusicDisplayModeRecordVertical;
        bool isRecordHorizontal = normalizedMode == MusicEnvironmentContext.MusicDisplayModeRecordHorizontal;
        ...
        if (isRecordVertical)
        {
            ...
            QueueTitleMarqueeUpdate();
            MusicUiContext.TryEnqueue(DispatcherQueue, UpdateProgressVisuals);
            return;
        }
    ...
    {
        return MusicEnvironmentContext.NormalizeMusicDisplayMode(displayMode) switch
        {
            MusicEnvironmentContext.MusicDisplayModeCover => true,
            MusicEnvironmentContext.MusicDisplayModeControls => false,
        ...
        string? sessionId)
    {
        try
        {
            ...
            if (!selected)
            {
                PackageLog.Write("[MusicWidget] Selected source closed before it could be activated.");
            }
        }
        catch (Exception ex)
        {
            PackageLog.Write($"[MusicWidget] Select source failed: {ex}");
        }
    }
    ...
    private void WireRecordProgress(Grid grid)
    {
        PointerEventHandler pressed = (s, e) => MusicUiContext.Invoke(() => ProgressHost_PointerPressed(s, e));
        PointerEventHandler moved = (s, e) => MusicUiContext.Invoke(() => ProgressHost_PointerMoved(s, e));
        PointerEventHandler released = (s, e) => MusicUiContext.Invoke(() => ProgressHost_PointerReleased(s, e));
        grid.PointerPressed += pressed; grid.PointerMoved += moved; grid.PointerReleased += released;
        _detachViewHandlers.Add(() => { grid.PointerPressed -= pressed; grid.PointerMoved -= moved; grid.PointerReleased -= released; });
    }
    ...
    private void QueueArtworkTransition()
    {
        ...
        {
            _ = MusicUiContext.TryEnqueue(DispatcherQueue, QueueArtworkTransition);
            return;
        }
        ...
        _ = MusicUiContext.TryEnqueue(DispatcherQueue, () =>
        {
    ...
    private async Task RunDeferredTitleMarqueeUpdateAsync(int version)
    {
        ...
        _ = MusicUiContext.TryEnqueue(DispatcherQueue, () => UpdateTitleMarquee(version));
    }
    ...
    private bool AreSystemAnimationsEnabled() =>
        ViewModel?.Environment.AllowSystemAnimations ?? true;

    private bool TextMarqueeAnimationsEnabled() =>
        ViewModel?.Environment.AllowTextMarqueeAnimations ?? true;

    private bool VinylRotationAnimationsEnabled() =>
        ViewModel?.Environment.AllowVinylRotationAnimations ?? true;
    ...
    {
        Grid track = _dragProgressHost ?? ProgressHost;
        if (ViewModel is null || track.ActualWidth <= 0)
        {
            return;
        }

        double x = e.GetCurrentPoint(track).Position.X;
        double ratio = Math.Clamp(x / track.ActualWidth, 0.0, 1.0);
