...
### 2026-09-10：Glance 实例设置实时同步

- 宿主保存成功后，经 `ILegacyInstanceMigration.SubscribeChanges` 通知对应的原生实例；通知在 UI dispatcher 合并执行，先同步完整配置快照，再发送现有 ABI v4 `RefreshRequested`。其他实例不受影响，销毁会解除订阅，已排队回调失效，复制失败不刷新旧快照。无需新增 HostApi 槽位或修改冻结签名。
- 包端重读配置后更新布局、时间显示、背景、照片按钮和历法开关；普通显示设置不重新打乱图片顺序，暂停状态继续保留。宿主仍是切换前的数据权威，包端不再将“受理但未持久化”的乐观设置写入磁盘缓存。异步保存失败的即时错误反馈和回滚确认仍待后续完成。
- 日历已实例化单元格随配置和语言变化重新应用日期装饰。真实窗口检查发现包漏带两行日期模板，并遗漏展开日历的时间/日期/星期区；本批补齐这些 XAML 内容，避免日期显示成小点、展开日历上方空白。
- 验证：x64 全量测试 3632/3632；Debug 构建及原生包签名验证通过；x64 NativeAOT 审计通过（源码稳定、AlwaysThrow=0、WMC1510=867/870、Rust staging/publish 一致）。隔离目录中加载最终原生包，两个实例分别同步，修改 A 不改变 B；用户截图确认日期、时间和“中秋”显示正常。此为开发验证，尚非正式渠道验收。
- 后续顺序：先完成 Glance 在线/Bing 图源、主题与日历材质、性能策略和真实输入/合并/叠放回归；随后以 Glance 打通私有 Store flight 和 Direct 包交付，再迁移 Music → Search → Weather → Todo → QuickCapture。包管理界面、正式发布者、1.5.0 一次升级的数据与离线包供应仍是发布阻塞项。

以上是本批增量，不能作为六功能已完成拆分或渠道已验收的依据。

### 2026-09-10：Glance 主题和日历材质

- 宿主配置 JSON 增加有效明暗主题、真实强调色、经系统兼容处理的材质类型、透明度和强度；沿用 HostApi v4 的既有槽位。主题/强调色变化触发会话通知，材质预览沿既有 `AppearanceChanged` 实例事件更新。
- 原生包原地更新自有笔刷，保留已加载 XAML 和日期模板的资源引用；无图背景跟随明暗主题，有图的时间文字保持可读，日历继续跟随宿主主题。
- 日历“跟随系统”接入 Mica/MicaAlt/Acrylic/AcrylicBase/Solid 的现有计算规则；“跟随图片”接入当前图两色取样和实例透明度。包拥有取色与材质实现，不引用宿主程序集；切图、离开图片模式、卸载视图或销毁时取消过期取色。
- 本批自动化覆盖宿主写出到真实包读取、旧/坏配置降级、材质计算与内置实现对照、图片取色对照及实例材质字段归一化。最终 x64 全量测试 3643/3643，Debug 构建及原生包签名验证通过；x64 AOT 审计通过（源码稳定、AlwaysThrow=0、WMC1510=867/870、Rust staging/publish 一致）。
- 用户操作及截图确认：深色/浅色切换、紫红到绿色强调色实时切换、纯色到云母材质变化、暖色到冷色图片取色联动；A 跟随宿主材质，B 跟随图片，两实例互不串改。对应 Debug 宿主加载已验证的原生 AOT 包，运行日志仅有初始两次创建，无原生包错误。仍需正式 AOT 宿主及 Direct/Store 渠道矩阵验收，亚克力在不同系统上的实际模糊效果也保留到该矩阵。
- 余项继续按 Glance 在线/Bing 图源 → 性能策略 → 输入/合并/叠放 → Direct/私有 Store flight 推进，再逐个迁移其他五项功能。发布目标 1.6.0，完整版本列车调整见 PR #325；不安排 1.5.x 新包。

### 2026-09-10：在线图源、缓存迁移和运行时边界（本地集成）

- 四个 GPT-6 Astra 子智能体分别交付图源服务、旧缓存迁移、性能策略与解码、运行时审查修正，主智能体集成 Controller 和回归测试；按用户要求不推送本批代码或 PR。
- Glance 包拥有 Bing/Wikimedia/本地图源服务及 JSON sourcegen。缓存和网络服务按包会话共享；首次在线读取前异步导入旧缓存，单个实例取消不会取消其他实例的迁移等待。缓存路径为包数据根下 `cache/glance`，原缓存保持只读。
- 图片请求先显示缓存，再接收在线更新；切源/分类/卸载/销毁使过期结果失效。显式 Refresh 保留完整刷新含义，新增 flags bit 0 表示 settings-only 通知，避免普通字体/布局设置变化重新请求网络。解码成功后才提交图层，坏图保留有效画面或回退渐变，解码尺寸使用原有视口规则。
- `allowImageAutoRotation` 与 `allowDecorativeAnimations` 经现有环境 JSON 下发，事件10和过渡入口读取有效策略；设置浮层使用中英文资源，其他语言回退英文。
- 进程级模块登记修复“关闭最后格子后换 hash 绕过重启”的缺口。源码方向和内存验收边界详见 [原生功能包方向复核](native-package-direction-review.md)。
- 统一验证：x64 全量测试 **3734/3734**，Debug 构建 0 错误，Glance 原生 AOT 包完整性和签名链通过；宿主 x64 AOT 审计源码稳定、AlwaysThrow=0、WMC1510=867/870、Rust staging/publish 一致。宿主 JSON 清单仍冻结为 29 文件/67 调用/27 context owner，包服务单独核对自己的 sourcegen；备份测试改为等待完整可读的阶段文件后再修改源，消除“文件刚创建、尚未复制”的测试竞争。
- 用户截图确认 Bing 和 Wikimedia 实际图片显示正常，随后确认轮播验证无问题。隔离测试包缓存包含 2 张旧缓存图及 6 张新下载图，旧缓存三文件 SHA-256 保持不变。左实例保持不变时查明其间隔仍为关闭，右实例为 10 秒，符合实例独立配置。
- 上述属于本地开发验证；正式 AOT 宿主的同包多实例/多包内存测量、Direct/Store、ARM64、输入/合并/叠放及完整翻译覆盖仍未完成。

### 2026-09-10：日历、多语言和胶囊背景（本地集成）

- 日历导航按正在浏览的月份重建农历/节日与传统标题；移植内置的整月滚轮、年/十年视图返回同步、1900–2100 范围和文化周起始日。明暗/语言变化保留浏览月份，年月视图隐藏不适用的传统标题。
- 包的七个实际 UI 文本键覆盖宿主全部 12 种文化，复用已有产品译文；en-US 基础表按键被目标语言覆盖，缺失或损坏译文不会污染其它键。此项是当前 Glance 表面的覆盖，不能代替后续其它功能包的翻译验收。
- 接入交互缩放和响应式布局开始/完成/取消事件：使用目标布局，忽略动画中间尺寸，取消恢复原尺寸；交互期间暂停图片轮播和重复解码。照片按钮按悬停或键盘焦点显示。
- 新增可选 `deskbox_widget_get_compact_background`，通过有 Size/Version 的 v1 结构返回 owned WinRT ImageSource 和实例透明度；宿主恰好一次释放 ABI 引用。旧包缺导出继续使用后备表现，现有 mandatory ABI/HostApi v4 签名不变；不会为胶囊重新解码或读取包内部 XAML。
- 胶囊背景经宿主通用接口消费，内容背景变化通知用于重新获取快照；初始为胶囊时也允许准备图片。胶囊的功能专用标题/摘要仍需后续独立语义快照，目前原生内容使用通用后备文本，尚未宣称完整胶囊表现对齐。
- 验证结果待本批统一构建和用户交互截图补记。保持本地未提交、未推送状态。
