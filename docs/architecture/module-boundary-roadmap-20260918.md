...
3. **IFeatureRuntime 替代记录**：§2 目标架构的统一功能运行时租约接口未正式立项；第 1/3/5/6 批按功能手写等价语义（幂等启停、可取消、串行化），第 23 批以 `ShutdownStep.Bounded` + 所有权中止收口退出侧。云同步立项时应正式立此接口（Sync 将是第四个功能运行时），届时以其实现替代各处手写。
