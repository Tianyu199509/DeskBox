...
| **1.5（v1.1 新增）** | **边界纪律落地（零拆分方案）**：功能代码移入 `Features/{Xxx}/` 目录 + namespace 纪律 + 新增 `ArchitectureContractTests`（枚举功能文件集，断言 using 白名单 ⊆ 契约/宿主 API、禁 ambient `App.Current.WidgetManager`）——先例：AotStage5B4B2AContractTests 已做同型"不得引用"断言，零新机制 | 六功能的边界有测试看守，无程序集工程税 |
